...
1. 出口と向かい合う面に新チャンクを配置する際、まず既存チャンクと重ならないか判定
   する。重なった場合は最大 3 マスまでずらして再試行。
2. 重ならずに置けても、そのチャンクの出口方向に **さらに 2 チャンク** 連続で配置でき
   る余白があるかを事前に調べる。
3. 余白が足りなければチャンクを作り直し、出口の向きを変えて検証する。向きを変えても
   無理な場合は一辺を 2 マスずつ縮めて再試行。
4. それでも置けなければ、十字型の小さな「隙間埋めチャンク」を生成してつなげる。
